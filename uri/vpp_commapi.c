...
/* @brief fd offset to put us above standard Linux
 *
 * alagalah : Don't like this but it's quick and dirty
 *
 */
...
vpp_commapi_main_t vpp_commapi_main;
...
{
  vpp_commapi_main_t *um = &vpp_commapi_main;
...
{
  vpp_commapi_main_t *vcm = va_arg (*args, vpp_commapi_main_t *);
...
cut_through_thread_fn (void *arg)
{
  ...
  while (true)
      ...
      while (actual_transfer > 0)
	{
      ...
      if (PREDICT_FALSE (vcm->time_to_stop))
	break;
    }
...
/* static void uri_udp_slave_test (vpp_commapi_main_t * vcm) */
/* { */
...
/*   memcpy (cmp->uri, vcm->connect_uri, vec_len (vcm->connect_uri)); */
...
/*   session = pool_elt_at_index (vcm->sessions, vcm->cut_through_session_index); */
/*   rx_fifo = session->server_rx_fifo; */
/*   tx_fifo = session->server_tx_fifo; */

/*   before = clib_time_now (&vcm->clib_time); */

/*   vec_validate (vcm->rx_buf, vec_len (test_data) - 1); */
...
/*       bytes_to_read = vec_len(vcm->rx_buf) > bytes_to_read ? */
/* 	bytes_to_read : vec_len(vcm->rx_buf); */
...
/* 	  for (j = 0; j < rv; j++) */
/* 	    { */
/* 	      if (vcm->rx_buf[j] != ((bytes_received + j) & 0xff)) */
/* 		{ */
...
/*   after = clib_time_now (&vcm->clib_time); */
/*   delta = after - before; */
...
void
uri_api_hookup (vpp_commapi_main_t * vcm)
{
#define _(N,n)                                                  \
...
static void
init_error_string_table (vpp_commapi_main_t * vcm)
{
...
handle_event_queue (vpp_commapi_main_t * vcm)
{
  ...
  while (1)
	...
	default:
      ...
      if (PREDICT_FALSE (vcm->time_to_stop == 1))
	break;
...
int
vpp_init (vpp_commapi_main_t * vcm)
{
...
int
is_vpp_init(vpp_commapi_main_t * vcm)
{
  //Need to check bounds on client_id returned from VPP but for now will use this int
  if (vcm->vpp_connected)
    {
      return 0;
    }
...
DO_NOT_SANITIZE_ADDRESS_ATTRIBUTE static int
...
DO_NOT_SANITIZE_ADDRESS_ATTRIBUTE static int
libc_vioctl (int d, unsigned long int request, va_list ap)
{
  long int args[4];
  int rc;
  int i;
...
static FILE *
libc_fopen (const char *name, const char *mode)
{
...
static int
libc_vopen (const char *pathname, int flags, va_list ap)
{
  long int mode = 0;
  int fd;
...
static int
libc_open (const char *pathname, int flags, ...)
{
  va_list ap;
  int fd;
...
#ifdef HAVE_SIGNALFD
static int
libc_signalfd (int fd, const sigset_t * mask, int flags)
{
...
#ifdef HAVE_TIMERFD_CREATE
static int
libc_timerfd_create (int clockid, int flags)
{
...
static void
set_port (int family, int prt, struct swrap_address *addr)
{
  switch (family)
    {
    case AF_INET:
...
static size_t
socket_wrapper_max_sockets (void)
{
  const char *s;
  unsigned long tmp;
  char *endp;
...
socket_wrapper_init_sockets (void)
{
  ...
  if (sockets == NULL)
    {
      SWRAP_LOG (SWRAP_LOG_ERROR, "Failed to allocate sockets array.\n");
      exit (-1);
    }
...
static unsigned int
socket_wrapper_default_iface (void)
{
...
static int
convert_un_in (const struct sockaddr_un *un, struct sockaddr *in,
	       socklen_t * len)
{
  unsigned int iface;
  unsigned int prt;
  ...
  SWRAP_LOG (SWRAP_LOG_TRACE, "type %c iface %u port %u", type, iface, prt);
...
static int
convert_in_un_remote (struct socket_info *si, const struct sockaddr *inaddr,
		      struct sockaddr_un *un, int *bcast)
{
  char type = '\0';
  unsigned int prt;
  unsigned int iface;
...
static int
convert_in_un_alloc (struct socket_info *si, const struct sockaddr *inaddr,
		     struct sockaddr_un *un, int *bcast)
{
  char type = '\0';
  unsigned int prt;
...
#if 0				/* FIXME */
static bool
check_addr_port_in_use (const struct sockaddr *sa, socklen_t len)
{
  struct socket_info_fd *f;
...
static int
sockaddr_convert_to_un (struct socket_info *si,
			const struct sockaddr *in_addr,
			socklen_t in_len,
			struct sockaddr_un *out_addr,
			int alloc_sock, int *bcast)
{
...
static int
sockaddr_convert_from_un (const struct socket_info *si,
...
static uint8_t *
swrap_pcap_packet_init (struct timeval *tval,
			...
			int unreachable, size_t * _packet_len)
{
  uint8_t *base;
  uint8_t *buf;
...
static int
swrap_pcap_get_fd (const char *fname)
{
  static int fd = -1;
...
static uint8_t *
swrap_pcap_marshall_packet (struct socket_info *si,
			    const struct sockaddr *addr,
			    enum swrap_packet_type type,
			    const void *buf, size_t len, size_t * packet_len)
{
  ...
  return swrap_pcap_packet_init (&tv,
				 ...
				 len,
				 tcp_seqno,
				 tcp_ack, tcp_ctl, unreachable, packet_len);
}

static void
swrap_pcap_dump_packet (struct socket_info *si,
			const struct sockaddr *addr,
			enum swrap_packet_type type,
			const void *buf, size_t len)
{
  ...
  packet = swrap_pcap_marshall_packet (si, addr, type, buf, len, &packet_len);
  if (packet == NULL)
    {
      return;
    }
...
#ifdef HAVE_SIGNALFD
static int
swrap_signalfd (int fd, const sigset_t * mask, int flags)
{
  int rc;
...
static int
swrap_socket (int family, int type, int protocol)
{
  struct socket_info *si;
...
int
is_our_socket (int family, int type, int protocol)
{

  //alagalah NOTE can't REALLY return AF_UNIX libc_socket from here....
...
int
vpp_socket (int family, int type, int protocol)
{
  vpp_commapi_main_t * vcm = &vpp_commapi_main;
  int rv;
  socket_info_t * si = 0;
  ...
  // Are we connected to VPP ?
  rv = is_vpp_init (&vpp_commapi_main);
  if (rv == -1)
    {
      exit (1);
    }

  rv = is_our_socket (family, type, protocol);
  if (rv != 0)
    return rv;
...
static int
swrap_socketpair (int family, int type, int protocol, int sv[2])
{
  int rc;
...
#ifdef HAVE_TIMERFD_CREATE
static int
swrap_timerfd_create (int clockid, int flags)
{
  int fd;
...
int
accept (int s, struct sockaddr *addr, Psocklen_t addrlen)
#else
int
...
*/
static int
swrap_auto_bind (int fd, struct socket_info *si, int family)
{
  ...
  for (i = 0; i < SOCKET_MAX_SOCKETS; i++)
    {
      ...
      if (stat (un_addr.sa.un.sun_path, &st) == 0)
	continue;
  ...
  if (i == SOCKET_MAX_SOCKETS)
    {
      SWRAP_LOG (SWRAP_LOG_ERROR, "Too many open unix sockets (%u) for "
		 ...
		 SOCKET_MAX_SOCKETS,
		 type, socket_wrapper_default_iface (), 0);
      errno = ENFILE;
      return -1;
    }
...
static int
swrap_connect (int s, const struct sockaddr *serv_addr, socklen_t addrlen)
{
  int ret;
  struct swrap_address un_addr = {
    .sa_socklen = sizeof (struct sockaddr_un),
  ...
  if (si->type == SOCK_DGRAM)
  ...
  else
    {
      ...
      ret = libc_connect (s, &un_addr.sa.s, un_addr.sa_socklen);
    }

  SWRAP_LOG (SWRAP_LOG_TRACE,
	     "connect() path=%s, fd=%d", un_addr.sa.un.sun_path, s);
  ...
  if (ret == 0)
      ...
      if (si->bindname.sa_socklen > 0)
	{
	  ...
	  memcpy (&si->myname.sa.ss,
		  &si->bindname.sa.ss, si->bindname.sa_socklen);
...
static int
swrap_bind (int s, const struct sockaddr *myaddr, socklen_t addrlen)
{
  int ret;
  struct swrap_address un_addr = {
    .sa_socklen = sizeof (struct sockaddr_un),
  ...
  ret = sockaddr_convert_to_un (si,
				myaddr,
				addrlen, &un_addr.sa.un, 1, &si->bcast);
  if (ret == -1)
    return -1;
  ...
  SWRAP_LOG (SWRAP_LOG_TRACE,
	     "bind() path=%s, fd=%d", un_addr.sa.un.sun_path, s);
...
static int swrap_getsockname (int s, struct sockaddr *name,
			      socklen_t * addrlen);

static int
swrap_bindresvport_sa (int sd, struct sockaddr *sa)
{
...
int
bindresvport (int sockfd, struct sockaddr_in *sinp)
{
...
static FILE *
swrap_fopen (const char *name, const char *mode)
{
  FILE *fp;
...
static int
swrap_vopen (const char *pathname, int flags, va_list ap)
{
  int ret;
...
static int
swrap_getpeername (int s, struct sockaddr *name, socklen_t * addrlen)
{
  struct socket_info *si = find_socket_info (s);
  socklen_t len;
...
#ifdef HAVE_ACCEPT_PSOCKLEN_T
int
getpeername (int s, struct sockaddr *name, Psocklen_t addrlen)
#else
int
getpeername (int s, struct sockaddr *name, socklen_t * addrlen)
#endif
{
...
static int
swrap_getsockname (int s, struct sockaddr *name, socklen_t * addrlen)
{
  struct socket_info *si = find_socket_info (s);
  socklen_t len;
...
#ifdef HAVE_ACCEPT_PSOCKLEN_T
int
getsockname (int s, struct sockaddr *name, Psocklen_t addrlen)
#else
int
...
static int
swrap_getsockopt (int s, int level, int optname,
		  void *optval, socklen_t * optlen)
{
  struct socket_info *si = find_socket_info (s);
...
#ifdef HAVE_ACCEPT_PSOCKLEN_T
int
getsockopt (int s, int level, int optname, void *optval, Psocklen_t optlen)
#else
int
...
static int
swrap_setsockopt (int s, int level, int optname,
		  const void *optval, socklen_t optlen)
{
  struct socket_info *si = find_socket_info (s);
...
static int
swrap_vioctl (int s, unsigned long int r, va_list va)
{
  struct socket_info *si = find_socket_info (s);
  va_list ap;
  int value;
...
#else
int
ioctl (int s, unsigned long int r, ...)
#endif
{
  va_list va;
  int rc;
 ...
 */
static void
swrap_msghdr_add_cmsghdr (struct msghdr *msg,
...
static int
swrap_msghdr_add_pktinfo (struct socket_info *si, struct msghdr *msg)
{
  /* Add packet info */
  switch (si->pktinfo)
    {
...
static int
swrap_msghdr_add_socket_info (struct socket_info *si, struct msghdr *omsg)
{
  int rc = 0;
...
static int
swrap_sendmsg_filter_cmsghdr (struct msghdr *msg,
			      uint8_t ** cm_data, size_t * cm_data_space)
{
  struct cmsghdr *cmsg;
  int rc = -1;
  ...
  for (cmsg = CMSG_FIRSTHDR (msg);
	...
	case IPPROTO_IP:
	  rc = swrap_sendmsg_filter_cmsg_socket (cmsg,
						 cm_data, cm_data_space);
	  break;
	default:
	  rc = swrap_sendmsg_copy_cmsg (cmsg, cm_data, cm_data_space);
	  break;
	}
    }
...
static int
swrap_sendmsg_copy_cmsg (struct cmsghdr *cmsg,
			 uint8_t ** cm_data, size_t * cm_data_space)
{
  size_t cmspace;
  uint8_t *p;
...
static int
swrap_sendmsg_filter_cmsg_socket (struct cmsghdr *cmsg,
				  uint8_t ** cm_data, size_t * cm_data_space)
{
  int rc = -1;
...
#ifdef IP_PKTINFO
    case IP_PKTINFO:
      rc = swrap_sendmsg_filter_cmsg_pktinfo (cmsg, cm_data, cm_data_space);
      break;
#endif
#ifdef IPV6_PKTINFO
    case IPV6_PKTINFO:
...
static int
swrap_sendmsg_filter_cmsg_pktinfo (struct cmsghdr *cmsg,
				   uint8_t ** cm_data, size_t * cm_data_space)
{
  (void) cmsg;			/* unused */
  (void) cm_data;		/* unused */
...
swrap_sendmsg_before (int fd,
		      ...
		      const struct sockaddr **to, int *bcast)
{
  size_t i, len = 0;
  ssize_t ret;
  ...
  switch (si->type)
	...
	if (msg->msg_iovlen == 0)
	  {
	    *tmp_iov = msg->msg_iov[0];
	    tmp_iov->iov_len = MIN ((size_t) tmp_iov->iov_len, (size_t) mtu);
	    msg->msg_iov = tmp_iov;
	    msg->msg_iovlen = 1;
	  }
	break;
      ...
      if (!si->defer_connect)
	{
      ...
      ret = sockaddr_convert_to_un (si,
				    &si->peername.sa.s,
				    si->peername.sa_socklen, tmp_un, 0, NULL);
      if (ret == -1)
	return -1;
...
static void
swrap_sendmsg_after (int fd,
		     struct socket_info *si,
		     struct msghdr *msg,
		     const struct sockaddr *to, ssize_t ret)
{
  ...
  for (i = 0; i < (size_t) msg->msg_iovlen; i++)
    {
      ...
      memcpy (buf + ofs, msg->msg_iov[i].iov_base, this_time);
      ofs += this_time;
      remain -= this_time;
    }
  len = ofs;
...
static int
swrap_recvmsg_before (int fd,
		      struct socket_info *si,
		      struct msghdr *msg, struct iovec *tmp_iov)
{
  size_t i, len = 0;
  ssize_t ret;
...
static int
swrap_recvmsg_after (int fd,
		     struct socket_info *si,
		     struct msghdr *msg,
		     const struct sockaddr_un *un_addr,
		     socklen_t un_addrlen, ssize_t ret)
{
  int saved_errno = errno;
  size_t i;
  uint8_t *buf = NULL;
  ...
  switch (si->type)
      ...
      if (un_addr != NULL)
	{
	  swrap_pcap_dump_packet (si,
				  msg->msg_name, SWRAP_RECVFROM, buf, ret);
	}
      else
	{
	  swrap_pcap_dump_packet (si, msg->msg_name, SWRAP_RECV, buf, ret);
	}
...
#ifdef HAVE_STRUCT_MSGHDR_MSG_CONTROL
  if (rc == 0 && msg->msg_controllen > 0 && msg->msg_control != NULL)
    {
...
static ssize_t
swrap_recvfrom (int s, void *buf, size_t len, int flags,
		struct sockaddr *from, socklen_t * fromlen)
{
  ...
  ret = libc_recvfrom (s,
		       buf,
		       len, flags, &from_addr.sa.s, &from_addr.sa_socklen);
  if (ret == -1)
    {
      return ret;
    }
  ...
  tret = swrap_recvmsg_after (s,
			      si,
			      &msg,
			      &from_addr.sa.un, from_addr.sa_socklen, ret);
  if (tret != 0)
    {
      return tret;
    }
...
static ssize_t
swrap_sendto (int s, const void *buf, size_t len, int flags,
	      const struct sockaddr *to, socklen_t tolen)
{
  ...
  rc = swrap_sendmsg_before (s,
			     si,
			     &msg, &tmp, &un_addr.sa.un, &to_un, &to, &bcast);
  if (rc < 0)
    {
      return -1;
    }
  ...
  if (bcast)
    {
      ...
      unsigned int iface;
      unsigned int prt =
	ntohs (((const struct sockaddr_in *) (const void *) to)->sin_port);
      char type;
      ...
      for (iface = 0; iface <= MAX_WRAPPED_INTERFACES; iface++)
	{
	  ...
	  libc_sendto (s, buf, len, flags, &un_addr.sa.s, un_addr.sa_socklen);
	}
  ...
  if (si->type == SOCK_DGRAM && si->connected)
    {
      ret = libc_sendto (s, buf, len, flags, NULL, 0);
    }
  else
    {
      ret = libc_sendto (s,
...
static ssize_t
swrap_recv (int s, void *buf, size_t len, int flags)
{
  struct socket_info *si;
  struct msghdr msg;
  struct swrap_address saddr = {
...
static ssize_t
swrap_read (int s, void *buf, size_t len)
{
  struct socket_info *si;
  struct msghdr msg;
  struct iovec tmp;
...
ssize_t
read (int s, void *buf, size_t len)
{
...
static ssize_t
swrap_write (int s, const void *buf, size_t len)
{
  struct msghdr msg;
  struct iovec tmp;
  struct sockaddr_un un_addr;
...
ssize_t
write (int s, const void *buf, size_t len)
{
...
static ssize_t
swrap_send (int s, const void *buf, size_t len, int flags)
{
  struct msghdr msg;
  struct iovec tmp;
  struct sockaddr_un un_addr;
...
static ssize_t
swrap_recvmsg (int s, struct msghdr *omsg, int flags)
{
  struct swrap_address from_addr = {
    .sa_socklen = sizeof (struct sockaddr_un),
  };
  ...
  if (si->type == SOCK_STREAM)
    {
      omsg->msg_namelen = 0;
	   ...
	   omsg->msg_namelen != 0 && omsg->msg_namelen >= msg.msg_namelen)
    {
...
static ssize_t
swrap_sendmsg (int s, const struct msghdr *omsg, int flags)
{
  struct msghdr msg;
  struct iovec tmp;
  struct sockaddr_un un_addr;
  ...
  rc =
    swrap_sendmsg_before (s, si, &msg, &tmp, &un_addr, &to_un, &to, &bcast);
  if (rc < 0)
    {
      return -1;
    }
  ...
  if (bcast)
      ...
      for (iface = 0; iface <= MAX_WRAPPED_INTERFACES; iface++)
	{
	  snprintf (un_addr.sun_path, sizeof (un_addr.sun_path),
		    "%s/" SOCKET_FORMAT, socket_wrapper_dir (), type, iface,
		    prt);
	  if (stat (un_addr.sun_path, &st) != 0)
	    continue;
...
ssize_t
sendmsg (int s, const struct msghdr * omsg, int flags)
{
...
static ssize_t
swrap_readv (int s, const struct iovec *vector, int count)
{
  struct socket_info *si;
  struct msghdr msg;
  struct iovec tmp;
...
ssize_t
readv (int s, const struct iovec * vector, int count)
{
...
static ssize_t
swrap_writev (int s, const struct iovec *vector, int count)
{
  struct msghdr msg;
  struct iovec tmp;
  struct sockaddr_un un_addr;
...
ssize_t
writev (int s, const struct iovec * vector, int count)
{
...
static int
swrap_vfcntl (int fd, int cmd, va_list va)
{
  struct socket_info_fd *src_fi, *fi;
  struct socket_info *si;
  int rc;
...
#ifdef HAVE_EVENTFD
static int
swrap_eventfd (int count, int flags)
{
  int fd;
