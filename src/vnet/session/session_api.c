...
static int
send_session_connected_uri_callback (u32 api_client_index, stream_session_t *s,
				     u8 is_fail)
{
...
vl_api_bind_uri_t_handler (vl_api_bind_uri_t * mp)
{
  ...
  _Static_assert(sizeof(u64) * SESSION_OPTIONS_N_OPTIONS <= sizeof (mp->options),
...
vl_api_bind_t_handler (vl_api_bind_t *mp)
{
  ...
  STATIC_ASSERT (sizeof(u64) * SESSION_OPTIONS_N_OPTIONS <= sizeof (mp->options),
      "Out of options, fix api message definition");
  ...
  clib_memcpy (&a->tep.ip, mp->ip,
	       (mp->is_ip4 ? sizeof(ip4_address_t) : sizeof(ip6_address_t)));
