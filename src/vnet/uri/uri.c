...
void
vpp_session_event_queue_allocate (stream_server_main_t *ssm, u32 thread_index)
{
  ...
  if (ssm->vpp_event_queues[thread_index] == 0)
    {
      ...
      ssm->vpp_event_queues[thread_index] = unix_shared_memory_queue_init (
                                    ...
                                    transport_connection_t * tc, u64 value)
{
  ...
  switch (sst)
    ...
    case SESSION_TYPE_IP4_TCP:
      make_v4_ss_kv_from_tc (&kv4, tc);
      kv4.value = value;
      clib_bihash_add_del_16_8 (&ssm->v4_half_open_hash, &kv4, 1 /* is_add */);
      break;
    case SESSION_TYPE_IP6_UDP:
    case SESSION_TYPE_IP6_TCP:
      make_v6_ss_kv_from_tc (&kv6, tc);
      kv6.value = value;
      clib_bihash_add_del_48_8 (&ssm->v6_half_open_hash, &kv6, 1 /* is_add */);
      break;
    default:
      clib_warning("Session type not supported");
      ASSERT(0);
                                    ...
                                    transport_connection_t *tc)
{
  ...
  switch (sst)
    ...
    case SESSION_TYPE_IP4_TCP:
      make_v4_ss_kv_from_tc (&kv4, tc);
      clib_bihash_add_del_16_8 (&ssm->v4_half_open_hash, &kv4, 0 /* is_add */);
      break;
    case SESSION_TYPE_IP6_UDP:
    case SESSION_TYPE_IP6_TCP:
      make_v6_ss_kv_from_tc (&kv6, tc);
      clib_bihash_add_del_48_8 (&ssm->v6_half_open_hash, &kv6, 0 /* is_add */);
      break;
    default:
      clib_warning("Session type not supported");
      ASSERT(0);
...
static int
stream_server_add_first_segment (stream_server_main_t *ssm, stream_server_t *ss,
                                 u32 segment_size, u8 **segment_name)
{
  svm_fifo_segment_create_args_t _ca, *ca = &_ca;
  memset (ca, 0, sizeof (*ca));
          ...
          if (!session->is_deleted)
            {
              ...
              vec_add1(deleted_sessions, session - ssm->sessions[thread_index]);
                      ...
                      ip46_address_t *ip, u16 port)
{
  ...
  /* Add to the main lookup table */
  stream_session_table_add_for_tc (ssm, s->session_type, tc, s->session_index);
...
stream_session_connect_notify (transport_connection_t *tc, u8 sst, u8 code)
{
  ...
  fifo_segment = svm_fifo_get_segment (
      ss->segment_indices[new_s->server_segment_index]);
...
stream_session_accept_notify (transport_connection_t *tc)
{
  ...
  ss->session_accept_callback (ss, s, ssm->vpp_event_queues[tc->thread_index]);
}
...
void
stream_session_open (stream_server_main_t *ssm, u8 sst, ip46_address_t *addr,
                  ...
                  char *segment_name, u32 *name_length, void *mp)
{
  ...
  if (s && (ss->flags & URI_OPTIONS_FLAGS_USE_FIFO) == 1)
    return stream_server_connect_to_local_server (ss, &ip46_address, mp, is_ip4);
